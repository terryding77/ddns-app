...
def signature(sign_to_string, sign_secret_key):
    h = hmac.new(sign_secret_key.encode('utf8'), sign_to_string.encode('utf8'), hashlib.sha1)
...
def signature_with_public_prama(params):
    ...
    sorted_params = map(lambda param : '='.join(map(percent_encode, param)), sorted(params.items()))
    canonicalized_query_string = '&'.join(sorted_params)
    sign_to_string = cnf.REQUEST_METHOD + "&%2F&" + percent_encode(canonicalized_query_string)
...
def alidns_request(params=None, **param_dict):
    ...
    response = getattr(requests, cnf.REQUEST_METHOD.lower())(ALIDNS_SITE, params=params)
    return response 
...
def get_dns_list(**param_dict):
    ...
    for domain_name, prefixs in cnf.ALIYUN_DOMAIN_NAME_DICT.items():
        for prefix in prefixs.split(','):
            prefix = prefix.strip()
            res = alidns_request(Action='DescribeDomainRecords', DomainName=domain_name, RRKeyWord=prefix)
            print(res.json())
...
def update_dns(domain_name, ip_address, prefix, record_id, **param_dict):
    if record_id:
        response = alidns_request(Action='UpdateDomainRecord', RecordId=record_id, RR=prefix, Type='A', Value=ip_address, **param_dict)
    else:
        response = alidns_request(Action='AddDomainRecord', DomainName=domain_name, RR=prefix, Type='A', Value=ip_address, **param_dict)
    print(response.text)
    return response

if __name__ == '__main__':
    #for (k, v) in signature_with_public_prama({'Action':'DescribeDomains'}).items(): print(k + ':'+v)
    for x in get_dns_list(): print(x)
